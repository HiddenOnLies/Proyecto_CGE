...
class MedidorRepoImpl(
    // Dependencia privada for el almacenamiento genérico.
    ...
    // Elimina TODOS los medidores asociados a un cliente, y también el índice.
    override fun eliminarPorCliente(rutCliente: String) {
        // 1. Obtiene la lista de IDs de medidores para el cliente.
        val medidorIds = obtenerIdsDeCliente(rutCliente)

        // 2. Elimina cada archivo de medidor individualmente.
        medidorIds.forEach { id ->
            persistencia.delete(prefix + id)
        }

        // 3. Elimina el archivo de índice que asocia al cliente con sus medidores.
        persistencia.delete(indexClienteMedidor + rutCliente)
    }
}